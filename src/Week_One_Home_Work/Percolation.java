...
public class Percolation {
	...
	public boolean isFull(int row, int col) {// is site (row i, column j) full?
		...
		if (row == n) {
			if (percolation[col - 1] != 1){
				if(isOpen(row , col) && checkPer(row , col)) {	
					percolation[col - 1] = 1;
					return true;
				} 
			}
	...
	private boolean checkPer(int i , int j) {
		if ((i - 1) > 0 && isOpen(i - 1 , j) && connected(toNum(i - 1, j) , 0)) {
			return true; 
		}if ((j - 1) > 0 && isOpen(i , j - 1) && percolation[j - 1] == 1) {
			return true;
		} if ((j + 1) < n && isOpen(i , j + 1) && percolation[j + 1] == 1) {
			return true;
		}
		return false;
	}
